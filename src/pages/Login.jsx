...
const Login = ({ client, alert, setAlert, open, setOpen, severity, setSeverity }) => {
  ...
  const onSubmit = async (e) => {
    ...
    if (res.error) {
      setAlert(res.error.data.detail);
      setOpen(true);
        ...
        <form onSubmit={onSubmit} method="post">
          <div className='auth-form-container'>
            <div className='auth-header'>
              <h2>Log In</h2>
              <div>
                {showLoginAlert && <CustomAlert message={alert} open={open} setOpen={setOpen} severity={severity} />}
              </div>
            </div>
